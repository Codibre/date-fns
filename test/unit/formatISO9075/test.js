...
describe('formatISO9075', () => {
    ...
    it('formats only date', () => {
        ...
        assert(formatISO9075(date, { representation: 'date', format: 'extended' }) ===
            '2019-12-11');
        assert(formatISO9075(date, { representation: 'date', format: 'basic' }) ===
            '20191211');
    });
    it('formats only time', () => {
        const date = new Date(2019, 2 /* Mar */, 3, 19, 0, 52, 123);
        assert(formatISO9075(date, { representation: 'time', format: 'extended' }) ===
            '19:00:52');
        assert(formatISO9075(date, { representation: 'time', format: 'basic' }) ===
            '190052');
    });
