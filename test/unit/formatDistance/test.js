...
describe('formatDistance', () => {
    describe('seconds', () => {
        describe('when the includeSeconds option is true', () => {
            it('less than 5 seconds', () => {
                const result = formatDistance(new Date(1986, 3, 4, 10, 32, 0), new Date(1986, 3, 4, 10, 32, 3), { includeSeconds: true });
                assert(result === 'less than 5 seconds');
            });
            it('less than 10 seconds', () => {
                const result = formatDistance(new Date(1986, 3, 4, 10, 32, 0), new Date(1986, 3, 4, 10, 32, 7), { includeSeconds: true });
                assert(result === 'less than 10 seconds');
            });
            it('less than 20 seconds', () => {
                const result = formatDistance(new Date(1986, 3, 4, 10, 32, 0), new Date(1986, 3, 4, 10, 32, 15), { includeSeconds: true });
                assert(result === 'less than 20 seconds');
            });
            it('half a minute', () => {
                const result = formatDistance(new Date(1986, 3, 4, 10, 32, 0), new Date(1986, 3, 4, 10, 32, 25), { includeSeconds: true });
                assert(result === 'half a minute');
            });
            it('less than a minute', () => {
                const result = formatDistance(new Date(1986, 3, 4, 10, 32, 0), new Date(1986, 3, 4, 10, 32, 45), { includeSeconds: true });
                assert(result === 'less than a minute');
            });
            it('1 minute', () => {
                const result = formatDistance(new Date(1986, 3, 4, 10, 32, 0), new Date(1986, 3, 4, 10, 33, 0), { includeSeconds: true });
    ...
    describe('when the addSuffix option is true', () => {
        it('adds a past suffix', () => {
            const result = formatDistance(new Date(1986, 3, 4, 10, 32, 0), new Date(1986, 3, 4, 10, 32, 25), { includeSeconds: true, addSuffix: true });
            assert(result === 'half a minute ago');
        });
        it('adds a future suffix', () => {
            const result = formatDistance(new Date(1986, 3, 4, 11, 32, 0), new Date(1986, 3, 4, 10, 32, 0), { addSuffix: true });
    ...
    describe('custom locale', () => {
        ...
        describe('does not contain `formatDistance` property', () => {
            it('throws `RangeError`', () => {
                const customLocale = {};
                const block = () => formatDistance(new Date(1986, 3, 4, 10, 32, 0), new Date(1986, 3, 4, 10, 32, 3), {
                    includeSeconds: true,
                    // @ts-expect-error
                    locale: customLocale,
                });
