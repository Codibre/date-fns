...
describe('differenceInCalendarDays', () => {
    it('returns the number of calendar days between the given dates', () => {
        const result = differenceInCalendarDays(new Date(2012, 6 /* Jul */, 2, 18, 0), new Date(2011, 6 /* Jul */, 2, 6, 0));
        assert(result === 366);
    });
    it('returns a negative number if the time value of the first date is smaller', () => {
        const result = differenceInCalendarDays(new Date(2011, 6 /* Jul */, 2, 6, 0), new Date(2012, 6 /* Jul */, 2, 18, 0));
        assert(result === -366);
    });
    it('accepts timestamps', () => {
        const result = differenceInCalendarDays(new Date(2014, 8 /* Sep */, 5, 18, 0).getTime(), new Date(2014, 8 /* Sep */, 4, 6, 0).getTime());
        assert(result === 1);
    });
    describe('edge cases', () => {
        it('the difference is less than a day, but the given dates are in different calendar days', () => {
            const result = differenceInCalendarDays(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 4, 23, 59));
            assert(result === 1);
        });
        it('the same for the swapped dates', () => {
            const result = differenceInCalendarDays(new Date(2014, 8 /* Sep */, 4, 23, 59), new Date(2014, 8 /* Sep */, 5, 0, 0));
            assert(result === -1);
        });
        it('the time values of the given the given dates are the same', () => {
            const result = differenceInCalendarDays(new Date(2014, 8 /* Sep */, 6, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
            assert(result === 1);
        });
