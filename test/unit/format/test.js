...
describe('format', () => {
    ...
    describe('day', () => {
        ...
        describe('day of year', () => {
            ...
            it('returns a correct day number for the last day of a leap year', () => {
                const result = format(new Date(1992, 11 /* Dec */, 31, 23, 59, 59, 999), 'D', { useAdditionalDayOfYearTokens: true });
                assert(result === '366');
            });
        });
    });
    ...
    describe('day period and hour', () => {
        ...
        describe('AM, PM, noon, midnight', () => {
            ...
            it('12 AM', () => {
                const date = new Date(1986, 3 /* Apr */, 6, 0, 0, 0, 900);
                assert(format(date, 'b bb bbb bbbb bbbbb') ===
    ...
    describe('custom locale', () => {
        ...
        it("throws `RangeError` if `options.locale` doesn't have `localize` property", () => {
            ...
            const block = () => format(date, 'yyyy-MM-dd', {
                // @ts-expect-error
                locale: customLocale,
            });
