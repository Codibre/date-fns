...
describe('differenceInISOWeekYears', () => {
    it('returns the number of full ISO week-numbering years between the given dates', () => {
        const result = differenceInISOWeekYears(new Date(2012, 6 /* Jul */, 2, 18, 0), new Date(2011, 6 /* Jul */, 2, 6, 0));
        assert(result === 1);
    });
    it('returns a negative number if the time value of the first date is smaller', () => {
        const result = differenceInISOWeekYears(new Date(2011, 6 /* Jul */, 2, 6, 0), new Date(2012, 6 /* Jul */, 2, 18, 0));
        assert(result === -1);
    });
    it('accepts timestamps', () => {
        const result = differenceInISOWeekYears(new Date(2014, 6 /* Jul */, 2).getTime(), new Date(2010, 6 /* Jul */, 2).getTime());
        assert(result === 4);
    });
    ...
    describe('edge cases', () => {
        ...
        it('the given dates are the same', () => {
            const result = differenceInISOWeekYears(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
            assert(result === 0);
        });
