...
describe('formatISO', () => {
    ...
    it('formats ISO-8601 basic format', () => {
        ...
        assert(formatISO(date, { format: 'basic' }) === `20191004T123013${tzOffsetBasic}`);
    });
    it('formats only date', () => {
        const date = new Date(2019, 11 /* Dec */, 11, 1, 0, 0, 789);
        assert(formatISO(date, { representation: 'date', format: 'extended' }) ===
            '2019-12-11');
        assert(formatISO(date, { representation: 'date', format: 'basic' }) ===
            '20191211');
    });
    it('formats only time', () => {
        const date = new Date(2019, 2 /* Mar */, 3, 19, 0, 52, 123);
        const tzOffset = generateOffset(date);
        assert(formatISO(date, { representation: 'time', format: 'extended' }) ===
            `19:00:52${tzOffset}`);
        assert(formatISO(date, { representation: 'time', format: 'basic' }) ===
            `190052${tzOffset}`);
    });
