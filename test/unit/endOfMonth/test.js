...
describe('endOfMonth', () => {
    it('returns the date with the time set to 23:59:59.999 and the date set to the last day of a month', () => {
        ...
        assert.deepStrictEqual(result, new Date(2014, 8 /* Sep */, 30, 23, 59, 59, 999));
    });
    ...
    describe('edge cases', () => {
        it('works for last month in year', () => {
            ...
            const result = endOfMonth(date);
            assert.deepStrictEqual(result, new Date(2014, 11 /* Dec */, 31, 23, 59, 59, 999));
        });
        it('works for last day of month', () => {
            const date = new Date(2014, 9 /* Oct */, 31);
            const result = endOfMonth(date);
            assert.deepStrictEqual(result, new Date(2014, 9 /* Oct */, 31, 23, 59, 59, 999));
        });
    });
