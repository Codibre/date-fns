...
describe('differenceInBusinessDays', () => {
    ...
    describe('edge cases', () => {
        it('the difference is less than a day, but the given dates are in different calendar days', () => {
            const result = differenceInBusinessDays(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 4, 23, 59));
            assert(result === 1);
        });
        it('the same for the swapped dates', () => {
            const result = differenceInBusinessDays(new Date(2014, 8 /* Sep */, 4, 23, 59), new Date(2014, 8 /* Sep */, 5, 0, 0));
            assert(result === -1);
        });
        it('the time values of the given dates are the same', () => {
            const result = differenceInBusinessDays(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 4, 0, 0));
            assert(result === 1);
        });
        it('the given dates are the same', () => {
            const result = differenceInBusinessDays(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
            assert(result === 0);
        });
