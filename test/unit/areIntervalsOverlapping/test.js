...
describe('areIntervalsOverlapping', () => {
    ...
    describe("when the time intervals don't overlap", () => {
        it('returns false for a valid non overlapping interval before another interval', () => {
            ...
            const isOverlapping = areIntervalsOverlapping({ start: initialIntervalStart, end: initialIntervalEnd }, { start: earlierIntervalStart, end: earlierIntervalEnd });
        ...
        it('returns false for a valid non overlapping interval after another interval', () => {
            ...
            const isOverlapping = areIntervalsOverlapping({ start: initialIntervalStart, end: initialIntervalEnd }, { start: laterIntervalStart, end: laterIntervalEnd });
            assert(isOverlapping === false);
        });
        it('returns false for a non overlapping same-day interval', () => {
            ...
            const isOverlapping = areIntervalsOverlapping({ start: initialIntervalStart, end: initialIntervalEnd }, { start: sameDayIntervalStart, end: sameDayIntervalEnd });
            assert(isOverlapping === false);
        });
        it('returns false for an interval differing by a few hours', () => {
            ...
            const isOverlapping = areIntervalsOverlapping({ start: initialIntervalStart, end: initialIntervalEnd }, {
        ...
        it("returns false for an interval with the same startDateTime as the initial time intervals's endDateTime", () => {
            ...
            const isOverlapping = areIntervalsOverlapping({ start: initialIntervalStart, end: initialIntervalEnd }, { start: oneDayOverlapIntervalStart, end: oneDayOverlapIntervalEnd });
            assert(isOverlapping === false);
        });
    ...
    describe('when the time intervals overlap', () => {
        it('returns true for an interval included within another interval', () => {
            ...
            const isOverlapping = areIntervalsOverlapping({ start: initialIntervalStart, end: initialIntervalEnd }, { start: includedIntervalStart, end: includedIntervalEnd });
        ...
        it('returns true for an interval overlapping at the end', () => {
            ...
            const isOverlapping = areIntervalsOverlapping({ start: initialIntervalStart, end: initialIntervalEnd }, { start: endOverlappingIntervalStart, end: endOverlappingIntervalEnd });
            assert(isOverlapping === true);
        });
        ...
        it('returns true for an interval including another interval', () => {
            ...
            const isOverlapping = areIntervalsOverlapping({ start: initialIntervalStart, end: initialIntervalEnd }, { start: includingIntervalStart, end: includingIntervalEnd });
            assert(isOverlapping === true);
        });
    });
    ...
    it('throws an exception if the start date of the initial time interval is after the end date', () => {
        const block = areIntervalsOverlapping.bind(null, { start: new Date(2016, 10, 7), end: new Date(2016, 10, 3) }, { start: new Date(2016, 10, 5), end: new Date(2016, 10, 15) });
        assert.throws(block, RangeError);
    });
    it('throws an exception if the start date of the compared time interval is after the end date', () => {
        const block = areIntervalsOverlapping.bind(null, { start: new Date(2016, 10, 3), end: new Date(2016, 10, 7) }, { start: new Date(2016, 10, 15), end: new Date(2016, 10, 5) });
        assert.throws(block, RangeError);
    });
    describe('when the inclusive option is true', () => {
        it("returns true for an interval with the same startDateTime as the initial time interval's endDateTime", () => {
            const oneDayOverlapIntervalStart = new Date(2016, 11, 3, 15, 0, 0);
            const oneDayOverlapIntervalEnd = new Date(2016, 11, 14, 13, 0, 0);
            const isOverlapping = areIntervalsOverlapping({ start: initialIntervalStart, end: initialIntervalEnd }, { start: oneDayOverlapIntervalStart, end: oneDayOverlapIntervalEnd }, { inclusive: true });
    ...
    describe('one of the dates is `Invalid Date`', () => {
        it('throws an exception if the start date of the initial time interval is `Invalid Date`', () => {
            const block = areIntervalsOverlapping.bind(null, { start: new Date(NaN), end: new Date(2016, 10, 3) }, { start: new Date(2016, 10, 5), end: new Date(2016, 10, 15) });
            assert.throws(block, RangeError);
        });
        it('throws an exception if the end date of the initial time interval is `Invalid Date`', () => {
            const block = areIntervalsOverlapping.bind(null, { start: new Date(2016, 10, 3), end: new Date(NaN) }, { start: new Date(2016, 10, 5), end: new Date(2016, 10, 15) });
            assert.throws(block, RangeError);
        });
        it('throws an exception if the start date of the compared time interval is `Invalid Date`', () => {
            const block = areIntervalsOverlapping.bind(null, { start: new Date(2016, 10, 3), end: new Date(2016, 10, 7) }, { start: new Date(NaN), end: new Date(2016, 10, 5) });
            assert.throws(block, RangeError);
        });
        it('throws an exception if the end date of the compared time interval is `Invalid Date`', () => {
            const block = areIntervalsOverlapping.bind(null, { start: new Date(2016, 10, 3), end: new Date(2016, 10, 7) }, { start: new Date(2016, 10, 5), end: new Date(NaN) });
            assert.throws(block, RangeError);
        });
    });
});
